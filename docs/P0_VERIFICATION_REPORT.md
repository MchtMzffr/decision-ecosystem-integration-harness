...
| run_one_step.py `_state_to_features`? | **OK** | Sadece `features = dict(state)` ve `now_ms` context'ten; domain anahtarı yok (yasak terim seti: `tests/test_invariant_0_domain_agnosticism.py` içinde tanımlı). |
| **C kararı** | **P0-C KAPALI** | Domain-agnostic passthrough. |
